...
/// based async APIs on <see cref="MsgPackSerializer"/>: they must fill/drain bounded buffers a chunk at a time
...
/// </summary>
public partial class MsgPackAsyncStreamingTests : TestBase
{
	...
	public async Task DeserializeAsync_Stream_ToleratesByteAtATimeReads()
	{
		...
		byte[] data = this.serializer.Serialize(original);
	...
	{
		byte[] data = this.serializer.Serialize(new Person("Ada"));
	...
	{
		// MessagePack values are always self-delimiting and require no separator whatsoever between them.
		Person[] people = [new Person("Ada"), new Person("Bob"), new Person("Cid")];
		using MemoryStream stream = new();
		foreach (Person person in people)
		{
			byte[] data = this.serializer.Serialize(person);
			stream.Write(data);
		}
		...
		await Assert.That(items.SequenceEqual(people)).IsTrue();
	}

	[Test]
	public async Task DeserializeAllAsync_ManyConcatenatedValues_ToleratesSegmentedPipeline()
	{
		// A longer run of concatenated values, fed through the pipe a few bytes at a time, exercises the scanner's
		// container-nesting bookkeeping (Person has a nested Address, itself skipped/recognized incrementally)
		// across many resumptions in a single scanner instance's lifetime.
		Person[] people = [.. Enumerable.Range(0, 50).Select(i => new Person($"Person{i}", $"padding-{i}"))];
		using MemoryStream stream = new();
		foreach (Person person in people)
		{
		...
		Task writeTask = Task.Run(async () =>
		{
			foreach (byte[] chunk in Chunk(data, 7))
			{
				await pipe.Writer.WriteAsync(chunk);
			}
		...
		List<Person?> items = [];
		await foreach (Person? item in this.serializer.DeserializeAllAsync<Person>(pipe.Reader))
		{
			items.Add(item);
		}
