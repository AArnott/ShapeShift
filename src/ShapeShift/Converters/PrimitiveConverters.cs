...
{
	internal static bool TryGetBuiltInConverter<T, TEncoder, TDecoder>([NotNullWhen(true)] out ShapeShiftConverter<T, TEncoder, TDecoder>? converter)
		where TEncoder : IEncoder, allows ref struct
		where TDecoder : IDecoder, allows ref struct
	{
		if (typeof(T) == typeof(string))
		{
			converter = (ShapeShiftConverter<T, TEncoder, TDecoder>)(object)new StringConverter<TEncoder, TDecoder>();
		}
		else if (typeof(T) == typeof(int))
		{
			converter = (ShapeShiftConverter<T, TEncoder, TDecoder>)(object)new Int32Converter<TEncoder, TDecoder>();
		}
		else
		{
			converter = null;
